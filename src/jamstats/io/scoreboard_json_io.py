...
# Columns to keep at the teamjam level
TEAMJAM_SUMMARY_COLUMNS = [
    "Calloff", "Injury", "JamScore", "Lead",
    "Lost", "NoInitial", "StarPass", "TotalScore", "jammer_name", "jammer_number"]

def read_game_data_json(filepath: str) -> Tuple[Dict[str, Any], pd.DataFrame]:
    """Read a json file into the data structures we're storing game data in.
    ...
    """
    pdf_game_state = jsonfile_to_pdf(filepath)
...
def jsonfile_to_pdf(filepath: str) -> pd.DataFrame:
    ...
    """
    game_json = json.load(open(filepath))
